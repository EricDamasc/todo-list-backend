...
def get_tasks(user: dict = Depends(get_current_user)):
    ...
    if "Items" not in response or not response["Items"]:
        ...
        raise HTTPException(
            status_code=200, detail="Nenhuma tarefa encontrada")
    ...
    print(f"Tarefas encontradas para o usuário")
    return response["Items"]
...
def create_tasks(tasks: List[TaskCreate], user: dict = Depends(get_current_user)):
    ...
    for task in tasks:
        ...
        print(f"Tarefa {task_id} criada para o usuário {user_id}")
...
def update_task(task_id: str, updated_task: TaskCreate, user: dict = Depends(get_current_user)):
    ...
    print(f"Atualizando tarefa {task_id} para o usuário {user_id}")
    ...
    if "Attributes" not in response:
        logger.warning(
            f"Tarefa {task_id} não encontrada para o usuário {user_id}")
        raise HTTPException(status_code=404, detail="Tarefa não encontrada")

    logger.info(f"Tarefa {task_id} atualizada para o usuário {user_id}")
    print(f"Tarefa {task_id} atualizada para o usuário {user_id}")
...
def delete_task(task_id: str, user: dict = Depends(get_current_user)):
    ...
    print(f"Deleta tarefa {task_id} para o usuário {user_id}")
    ...
    print(f"Tarefa {task_id} deletada para o usuário {user_id}")
